"""Manages the fleet of internal bot workers for mktbook_3."""
...
from mktbook_3.bots.bot_client import SingleBot
...
class BotFleet:
    """Manages all active internal bot workers (mktbook_3)."""
    ...
    async def start_bot(self, bot_row: Bot) -> None:
        ...
        log.info("Registered mktbook_3 bot %s (id=%d)", bot_row.bot_name, bot_row.id)

    async def stop_bot(self, bot_id: int) -> None:
        worker = self._bots.pop(bot_id, None)
        if worker:
            log.info("Unregistered mktbook_3 bot id=%d", bot_id)

    async def start_all(self) -> None:
        bots = await queries.get_active_bots(workout_id=3)
        for bot in bots:
